...
for idx_method = 1:num_method
    ...
    for idx_set = 1%:num_set
        ...
        for scale = [2 3 4]%scale_current
            filepaths = [];
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile(path_HR, dataset{idx_set}, ['x', num2str(scale)], ext{idx_ext})));
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                name_SR = strrep(name_HR, 'HR', method_name);
                im_HR = imread(fullfile(path_HR, dataset{idx_set}, ['x', num2str(scale)], name_HR));
                im_SR = imread(fullfile([methods{idx_method}], 'results', dataset{idx_set}, ['x', num2str(scale)], name_SR));
                % change channel
                ...
                fprintf('x%d %d %s: PSNR= %f SSIM= %f\n', scale, idx_im, name_HR, PSNR_all(idx_im), SSIM_all(idx_im));
            end
            fprintf('--------Mean--------\n');
